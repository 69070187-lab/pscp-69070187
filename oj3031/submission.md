...
```text
input:
พื้นที่ที่มีน้ำหมึก จำนวนคำร้องจากผู้คน 
พิกัดของแต่ละคน รับเป็น x,y

output:
วินาทีที่ถูกน้ำท่วม กับวินาทีที่คิดว่าจะน้ำท่วม (ของแต่ละคำร้องจากผู้คน)

Constraints:
ถ้าเวลาที่ท่วมพอดีไม่เป็นจำนวนเต็ม ให้คิดเป็นวินาทีถัดไป
```
...
```text
Step 1: รับค่า S กับ N ก่อน โดย S น่าจะเป็นปริมาณหมึกที่ใช้ได้ต่อหนึ่งครั้ง และ N คือจำนวนจุดที่ต้องพิมพ์
Step 2: วนลูปตามจำนวน N เพราะแต่ละจุดจะมีค่า x, y ของตัวเอง
Step 3: คิดพื้นที่วงกลมจากจุด (x, y) โดยใช้ π(x² + y²) เพราะระยะจากจุดกำเนิดถึง (x,y) คือ √(x²+y²) แล้วเอามายกกำลังสองในสูตรพื้นที่
Step 4: เอาพื้นที่ที่คำนวณได้หารด้วย S เพื่อดูว่าต้องใช้หมึกกี่ครั้ง แล้วใช้ ceil() เพราะถ้าออกมาเป็นเศษก็ต้องปัดขึ้น เช่น ต้องใช้ 2.1 ครั้ง ก็ใช้จริง 3 ครั้ง
Step 5: พิมพ์จำนวนครั้งที่ต้องใช้สำหรับแต่ละจุด

ตอนแรกผมคิดว่าต้องคำนวณระยะทางด้วย sqrt() ก่อน แต่พอลองดูสูตรพื้นที่แล้ว sqrt() กับการยกกำลังสองหักล้างกัน เลยใช้ x*x + y*y ได้ตรง ๆ ทำให้โค้ดสั้นกว่า

สิ่งที่ยังต้องระวังคือ PI ในโจทย์อาจกำหนดค่าไว้เฉพาะ ถ้าใช้ math.pi แทน 3.1416 ผลลัพธ์บางกรณีอาจต่างกันนิดหน่อย โดยเฉพาะตอนเอาไป ceil()
```
...
```text
วิธีผมทำสุดท้ายคือคำนวณพื้นที่จาก x กับ y แล้วหารด้วย S เพื่อดูว่าต้องใช้หมึกกี่ครั้ง จากนั้นใช้ ceil() เพื่อปัดขึ้น เพราะถ้ามีเศษก็ต้องใช้เพิ่มอีก 1 ครั้ง ตอนแรกผมไม่แน่ใจว่าต้องใช้ sqrt() ไหม แต่พอดูสูตรแล้วไม่จำเป็น เลยคำนวณ x*x + y*y ได้เลย
```
